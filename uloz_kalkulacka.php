...
if ($data) {
    $vaha = (float)$data['weight'];
    if ($vaha < 30 || $vaha > 200) {
        echo json_encode(['success' => false, 'message' => 'Neplatná váha.']);
        exit;
    }

    try {
        $cil = $data['goal'];
        if ($cil === 'udrzba') $cil = 'udrzovani';

        
        $sql = "INSERT INTO kalkulacka_vysledky 
        ...
        $stmt->execute([
            ...
            ':bmr'      => (int)$data['bmr'],
            ':vaha'     => (float)$data['weight'],
            ':aktivita' => (float)$data['activity'],
            ':tdee'     => (int)$data['tdee']
        ]);

        echo json_encode(['success' => true, 'message' => 'Výsledky byly uloženy!']);

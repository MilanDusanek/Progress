...
try {
    ...
    $check = $pdo->prepare("SELECT 1 FROM uzivatel_oblibene WHERE uzivatel_id = :uid AND potravina_id = :pid AND typ_jidla = :typ");
    $check->execute(['uid' => $uzivatel_id, 'pid' => $potravina_id, 'typ' => $typ_jidla]);

    if ($check->fetch()) {
        $stmt = $pdo->prepare("DELETE FROM uzivatel_oblibene WHERE uzivatel_id = :uid AND potravina_id = :pid AND typ_jidla = :typ");
        $stmt->execute(['uid' => $uzivatel_id, 'pid' => $potravina_id, 'typ' => $typ_jidla]);
        echo json_encode(['status' => 'removed']);
    } else {
        $stmt = $pdo->prepare("INSERT INTO uzivatel_oblibene (uzivatel_id, potravina_id, typ_jidla) VALUES (:uid, :pid, :typ)");
        $stmt->execute(['uid' => $uzivatel_id, 'pid' => $potravina_id, 'typ' => $typ_jidla]);
